...
void ComplexModel::draw(int width, int height, Shader_v2 &shader, Camera& camera, Light& light, GLuint shadow_map, bool render_shadow) {
	...
	for (unsigned int i = 0; i < meshes.size(); i++)
		meshes[i].draw(shader, camera, light, shadow_map, render_shadow);
}
...
vector<mTexture> ComplexModel::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
{
	vector<mTexture> textures;
