...
void main()
{
	vec3 result = spotLightRender(light, material, fragment_normal, fragment_position, view_position);
...
vec3 spotLightRender(Light light, Material material, vec3 fragmemt_normal, vec3 fragment_position, vec3 view_position)\
{
	...
	{
		result = vec3((light.ambient * material.ambient + light.diffuse * material.diffuse)*0.3);
	}
